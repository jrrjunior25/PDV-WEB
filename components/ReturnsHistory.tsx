...
const ReturnsHistory = () => {
  const { data: returns, loading, error, refetch } = useMockApi<Return[]>(api.getReturns);
  ...
  const renderTableContent = () => {
    if (loading) {
      return <tr><td colSpan={7} className="text-center py-8">Carregando devoluções...</td></tr>;
    }
    if (error) {
      return <tr><td colSpan={7} className="p-4"><ErrorDisplay message={error.message} onRetry={refetch} /></td></tr>;
    }
    if (returns?.length === 0) {
